...
use crate::{utils::{git, current_root_project, find}, JuliaProject};
...
impl Release {
    ...
    pub fn sync_with_remote(&self) -> Result<&Self, Error> {
        let repo = self.repo.as_ref().ok_or_else(|| format_err!("repo not found"))?;
        git::pull(repo)?;
        git::push(repo)?;
        Ok(self)
    }
    ...
    pub fn commit_changes(&self) -> Result<&Self, Error> {
        match self.version_spec {
            VersionSpec::Current => {}, // do nothing
            _ => {
                let version_to_release = self.version_to_release.as_ref()
                    .ok_or_else(|| format_err!("No version to release found"))?;
...
fn is_version_continuously_greater(latest: &Version, release: &Version) -> bool {
    ...
    // patch release
    if latest.major == release.major && latest.minor == release.minor &&
        latest.patch + 1 == release.patch {
            return true;
    }

    // minor release
    if latest.major == release.major && latest.minor + 1 == release.minor &&
        latest.patch == release.patch {
            return true;
    }

    // major release
    if latest.major + 1 == release.major && latest.minor == release.minor &&
        latest.patch == release.patch {
            return true;
    }
    false
